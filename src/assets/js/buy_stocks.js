async function fetch_stocks() { 
    const rawResponse = await fetch('http://stockmarketapi/stocks', {
        method: 'GET'})
    .then(res => res.json())
    .then(res => {return res});
    return rawResponse;
...
async function load_stocks() {
	...
	for(let i = 0; i < stocks.length; i++){
		$('#select_stocks').append(`${`<option value=${stocks[i].id}>${stocks[i].name}: USD${stocks[i].buy_price}</option>`}`)
	}
}
...
async function buy(){
    ...
    (async () => {
    	const rawResponse = await fetch('http://stockmarketapi/buy_stock', {
    		method: 'POST',
            headers: {
